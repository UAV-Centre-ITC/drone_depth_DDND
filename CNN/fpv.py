...
#URI = uri_helper.uri_from_env(default='tcp://aideck-B0A3E0.local:5000')
...
class MainWindow(QtWidgets.QWidget):
    ...
    def __init__(self, URI):
        ...
        self.labels = {
            ...
            'start_save': {
                'widget': QtWidgets.QLabel('save: {0}'.format(str(self.start_save_images))),
                'x_grid': 2, 'y_grid': 1,
                'alignment': QtCore.Qt.AlignLeft
            }
        }
    ...
    def updateImage(self, image):
        i = QtGui.QImage(image, CAM_WIDTH, CAM_HEIGHT, QtGui.QImage.Format_Grayscale8)
        if self.start_save_images:
            writer = QtGui.QImageWriter("./images/{0}.png".format(time.time()))
    ...
    def sendHoverCommandSlow(self):
        self.cf.high_level_commander.takeoff(self.hover['height'], duration_s=1)
    ...
    def startSave(self):
        self.start_save_images = not self.start_save_images
        self.labels['start_save']['widget'].setText('save: {0}'.format(str(self.start_save_images)))
