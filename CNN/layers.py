...
class ConvBlock(nn.Module):
    ...
    # def my_elu(self, x, gamma=1.):
    #     x = torch.where(x > 0, x, gamma * (torch.exp(x) - 1))
    #     return x
...
class Conv3x3(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, use_refl=True):
        ...
        if use_refl:
            ...
            self.pad = nn.ZeroPad2d(1)
        # self.conv = nn.Conv2d(int(in_channels), int(out_channels), 3)
        self.conv = nn.Conv2d(int(in_channels), int(out_channels), kernel_size=3, stride=1, padding=1, bias=False)
...
def scale_invariant_loss(input, target):
