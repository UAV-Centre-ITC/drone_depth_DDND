...
static pi_task_t task2;  //velocity
...
static CPXPacket_t txp2;
static CPXPacket_t txp3;
...
void createVelMsgPacket(CPXPacket_t * packet) {
  ...
  //(velMsg->header = (CRTP_PORT_GENERIC_SETPOINT <<2) | 0;
  velMsg->TYPE = 0x01;
  velMsg->vx = 0.1;
  velMsg->vy = 0.1;
  velMsg->vz = 0.1;
...
//cpxPrintToConsole(LOG_TO_CRTP, "bbbbbbbbbb status: %u\n", wifiClientConnected);
#if defined(USE_STREAMER)
    //cpxPrintToConsole(LOG_TO_CRTP, "aaaaaaaaaaa status: %u\n", wifiClientConnected);
    if (wifiClientConnected == 1)
    {
      // First send information about the image
      //cpxPrintToConsole(LOG_TO_CRTP, "creating image buffer.\n");


      createImageHeaderPacket(&txp, STREAM_W*STREAM_H, RAW_ENCODING);
      cpxSendPacketBlocking(&txp);
      // Send image
      sendBufferViaCPX(&txp, imgBuff0, STREAM_W*STREAM_H);
      //cpxPrintToConsole(LOG_TO_CRTP, "image buffer sent via cpx.\n");
      vTaskDelay(500);
    }
#endif
...
void velocity_task(void *parameters) {
  ...
  vTaskDelay(20000);
  //pi_time_wait_us(40000*1000);
  cpxPrintToConsole(LOG_TO_CRTP, "waiting done!\n");
  //send switching packet


  txp2.route.destination = CPX_T_STM32;
  //rxp2.route.source = CPX_T_GAP8;
  txp2.route.function = CPX_F_APP;
  txp2.route.version = CPX_VERSION;

  txp3.route.destination = CPX_T_STM32;
  //rxp2.route.source = CPX_T_GAP8;
  txp3.route.function = CPX_F_APP;
  txp3.route.version = CPX_VERSION;


  CPXCTRLPacket_t * cpxCtrl = (CPXCTRLPacket_t*) txp2.data;
  ...
  txp2.dataLength = sizeof(control_signal_connected);
  cpxSendPacketBlocking(&txp2);
  pi_time_wait_us(1000*1000);
  ...
  txp2.dataLength = sizeof(control_signal);
  cpxSendPacketBlocking(&txp2);
  ...
  while (1) {
    ...
    //txp3.data[0]=12;
    //txp3.data[1]=5;
    //txp3.dataLength = sizeof(txp3.data);
    createVelMsgPacket(&txp2);
    cpxPrintToConsole(LOG_TO_CRTP, txp2.dataLength);
    cpxSendPacketBlocking(&txp2);
    cpxPrintToConsole(LOG_TO_CRTP, "velocity has been sent!\n");
    vTaskDelay(500);
    pi_time_wait_us(1000*1000);
